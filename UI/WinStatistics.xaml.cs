    ...
    public partial class WinStatistics : Window
    {
        ...
        private void ApplyThemeColors()
        {
            try
            {
                ...
                if (windowFg != null)
                {
                    ...
                    if (rbWenlaiAll != null) rbWenlaiAll.Style = radioStyle;
                    ...
                    if (rbLocalAll != null) rbLocalAll.Style = radioStyle;
                    ...
                    if (rbTrainerAll != null) rbTrainerAll.Style = radioStyle;
        ...
        private async System.Threading.Tasks.Task LoadWenlaiStatistics(StatsRange range, DateTime? customStart = null, DateTime? customEnd = null)
        {
            try
            {
                ...
                {
                    var dates = WenlaiLog.GetAvailableSummaryDates();
                    if (dates.Count > 0)
                    {
                        startDate = dates.Min();
                ...
                if (customStart.HasValue) startDate = customStart.Value;
                if (customEnd.HasValue) endDate = customEnd.Value;

                // 使用新的 ReadStatisticsInRange 方法
                var stats = await Task.Run(() => WenlaiLog.ReadStatisticsInRange(startDate, endDate));
        ...
        private StatsRange GetSelectedWenlaiRange()
        {
            ...
            if (rbWenlaiAll != null && rbWenlaiAll.IsChecked == true) return StatsRange.All;
        ...
        private async System.Threading.Tasks.Task LoadLocalStatistics(StatsRange range)
        {
            try
            {
                ...
                if (range == StatsRange.All)
                {
                    var dates = ArticleLog.GetAvailableSummaryDates();
                    if (dates.Count > 0)
                    {
                        startDate = dates.Min();
                        endDate = DateTime.Now.Date.AddDays(1).AddTicks(-1);
                ...
                // 使用新的 ReadStatisticsInRange 方法
                var stats = await Task.Run(() => ArticleLog.ReadStatisticsInRange(startDate, endDate));
        ...
        private StatsRange GetSelectedLocalRange()
        {
            ...
            if (rbLocalAll != null && rbLocalAll.IsChecked == true) return StatsRange.All;
            return StatsRange.Today;
        }
        ...
        private async System.Threading.Tasks.Task LoadTrainerStatistics(StatsRange range)
        {
            try
            {
                ...
                if (range == StatsRange.All)
                {
                    var dates = TrainerLog.GetAvailableSummaryDates();
                    if (dates.Count > 0)
                    {
                        startDate = dates.Min();
                        endDate = DateTime.Now.Date.AddDays(1).AddTicks(-1);
                ...
                // 使用新的 ReadStatisticsInRange 方法
                var localStats = await Task.Run(() => TrainerLog.ReadStatisticsInRange(startDate, endDate));
        ...
        private StatsRange GetSelectedTrainerRange()
        {
            ...
            if (rbTrainerAll != null && rbTrainerAll.IsChecked == true) return StatsRange.All;
            return StatsRange.Today;
        }
