...
namespace Net
{
    public class Sender
    {
        ...
        public static string Post(string url, Dictionary<string, string> headers, string jsonBody)
        {
            url = Sender.FixUrl(url);
            // 添加header
            foreach (KeyValuePair<string, string> kvp in headers)
            ...
            StreamContent content = new StreamContent((Stream)new MemoryStream(Encoding.UTF8.GetBytes(jsonBody)));
            content.Headers.ContentType = new System.Net.Http.Headers.MediaTypeHeaderValue("application/json");
            content.Headers.ContentType.CharSet = "utf-8";

            Task<HttpResponseMessage> task = Sender.client.PostAsync(url, (HttpContent)content);
            try
            {
                return Sender.RunTask(task);
            }
