...
$snake = Get-Content -Path (Join-Path $root 'UI\Modes\SnakeMode.cs') -Raw
Assert-NotContains 'snake mode should not paint full-height correct block backgrounds' $snake 'tb.Background = _main.IsBlindType ? null : Colors.CorrectBackground'
Assert-NotContains 'snake mode should not paint full-height incorrect block backgrounds' $snake 'tb.Background = _main.IsBlindType ? null : Colors.IncorrectBackground'
Assert-Contains 'snake mode should route state backgrounds through main display helper' $snake '_main.SyncDisplayBlockStateBackground(i, background);'
Assert-Contains 'snake mode should animate only newly changed typed backgrounds' $snake '_main.SetDisplayBlockStateBackground(i, background);'
Assert-Contains 'snake mode should compare displayed state before choosing animated background updates' $snake 'TextInfo.BlocksStates[i] != state'
Assert-Contains 'snake mode should remember displayed state after background update' $snake 'TextInfo.BlocksStates[i] = state;'
Assert-Contains 'snake mode should rebuild when lower code-display visual tree is stale' $snake 'IsSnakeDisplayStaleForCodeDisplay(codeDisplayEnabled)'
Assert-Contains 'snake mode should treat missing lower code labels as stale while code display is enabled' $snake 'TextInfo.CodeLabels.Count <= i'
Assert-Contains 'snake mode should fade the full display wrapper so lower code follows snake visibility' $snake 'ApplySnakeOpacity(tb, opacity);'
Assert-NotContains 'snake mode should not fade only the source text block' $snake 'tb.Opacity = opacity;'
...
Assert-Contains 'copybook full typed-state recompute should resync unchanged visible backgrounds for code display' $copybookRefreshTypedStateBody '_main.ResyncDisplayBlockStateBackgroundByGlobalIndex(i, background);'
...
$setDisplayBackgroundBody = Get-MethodBody 'MainWindow.SetDisplayBlockStateBackground' $mainCode 'internal void SetDisplayBlockStateBackground(int localIndex, Brush background)' 'internal void SetDisplayBlockStateBackgroundByGlobalIndex'
Assert-Contains 'state background setter should keep using overlay when current visual tree has code-display backgrounds' $setDisplayBackgroundBody 'HasStateBackgroundOverlay(localIndex)'
$syncDisplayBackgroundBody = Get-MethodBody 'MainWindow.SyncDisplayBlockStateBackground' $mainCode 'internal void SyncDisplayBlockStateBackground(int localIndex, Brush background)' 'internal double GetDisplayStateBackgroundTopOffset'
Assert-NotContains 'snake bulk background sync should not delegate to animated state setter' $syncDisplayBackgroundBody 'SetDisplayBlockStateBackground(localIndex, background);'
Assert-Contains 'snake bulk background sync should resync overlay backgrounds without animation' $syncDisplayBackgroundBody 'ResyncStateBackgroundOverlay(localIndex, background);'
Assert-Contains 'snake bulk background sync should update non-overlay backgrounds without animation' $syncDisplayBackgroundBody 'SmoothBackground.Apply(block, background, 0);'
$resyncStateBackgroundBody = Get-MethodBody 'MainWindow.ResyncStateBackgroundOverlay' $mainCode 'private void ResyncStateBackgroundOverlay(int localIndex, Brush background)' 'private void EnsureCodeDisplayStateBackgrounds()'
Assert-NotContains 'state background resync should not ignore color changes after overlay already exists' $resyncStateBackgroundBody 'stateBackground.Background == null'
